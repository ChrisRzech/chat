          ...
          "[IncrementalRequestDeserializer]")
{
    ...
    const chat::common::ByteSpan serializedSpan{serialized.data(),
                                                serialized.size()};
    chat::messages::IncrementalRequestDeserializer deserializer;
    const auto result = deserializer.tryDeserialize(serializedSpan);
          ...
          "[IncrementalRequestDeserializer]")
{
    ...
    auto chunk = serializedSpan.subspan(chunkOffset, chunkSize);
    ...
    chunkSize = 1;
    chunk = serializedSpan.subspan(chunkOffset, chunkSize);
          ...
          "[IncrementalRequestDeserializer]")
{
    ...
    chat::messages::IncrementalRequestDeserializer deserializer;
    const auto result = deserializer.tryDeserialize(span);
    REQUIRE(!result.hasValue());
