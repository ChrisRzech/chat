...
void hexdump(std::ostream& out, const std::byte* bytes, std::size_t size)
{
    ...
    std::array<char, BYTES_PER_LINE + 1> chars = {'\0'}; //Make sure to have null-terminator
    ...
    for(std::size_t i = 0; i < size + linePaddingCount; i++)
    {
        ...
        {
            out << std::setw(8) << std::setfill('0') << std::hex << i << std::dec << std::setfill(' ') << "  ";
        }

        if(i < size)
        {
            out << std::setw(2) << std::setfill('0') << std::hex << static_cast<int>(bytes[i]) << std::dec << std::setfill(' ');
            chars[i % BYTES_PER_LINE] = std::isprint(static_cast<int>(bytes[i])) != 0 ? static_cast<char>(bytes[i]) : '.';
        }
        else
        {
            out << "  ";
